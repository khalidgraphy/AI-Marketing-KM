...
# By Kreative Minds (https://kreativeminds.ae)
...
if [ -n "$BASH_SOURCE" ] && [ "$BASH_SOURCE" != "bash" ] && [ -f "$BASH_SOURCE" ]; then
    ...
    TEMP_DIR=$(mktemp -d)
    git clone --depth 1 https://github.com/khalidgraphy/ai-marketing-km.git "$TEMP_DIR/ai-marketing-km" 2>/dev/null
    if [ $? -ne 0 ]; then
        echo -e "${RED}Failed to clone repository.${NC}"
        exit 1
    fi
    SCRIPT_DIR="$TEMP_DIR/ai-marketing-km"
fi
